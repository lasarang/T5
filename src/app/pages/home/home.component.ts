...
export class HomeComponent implements OnInit {
    ...
    this.authService.userData.pipe(
      switchMap(auth => {
        if (auth) {
          ...
          return this.firestoreService.getCollection(`users/${auth.uid}/contactos/`);
        } else {
          return [];
        }
      })
  ...
  deleteRowContactos(x: Contacto, i: any) {
    ...
    if (delBtn == true) {
      ...
      this.firestoreService.deleteDoc(`users/${this.usuario.id }/contactos/`, x.id);
    }
  }
  ...
  updateRowContactos(x: Contacto) {
    ...
    if (delBtn == true) {
      if (x.id == '' &&
        ...
        this.firestoreService.updateDoc(`users/${this.usuario.id }/contactos/`, x.id, x);
      }
